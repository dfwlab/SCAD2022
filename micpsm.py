...
class micpsm():
    ...
    def output_tables(self, target):
        ...
        r('as.data.frame(summary(psm)$nn)').to_csv(os.path.abspath(os.path.join(self.outpath, self.config['output']['sample_size_filename'])), sep='\t')
        ...
        return match_drop_unmatched, match, pairs, sum_all, sum_matched, balance_stats
    ...
    def propensity_score_match(self, data, target, features):
        ...
        match_drop_unmatched, match, pairs, sum_all, sum_matched, balance_stats = self.output_tables(target)
        self.output_figures(match, pairs, target, features, sum_all, sum_matched, balance_stats)
        return match_drop_unmatched, match, pairs, sum_matched, balance_stats
    ...
    def wilcoxon_signed_rank_test(self, x, y):
        # x, y 需为配对样本
    ...
    def diff_by_rank_sum(self, data, target, features):
        result = []
        for f in features:
            x = data.loc[data[target]==0, f]
            y = data.loc[data[target]==1, f]
            try:
                stat, p = self.wilcoxon_rank_sum_test(x, y)
            except:
                stat, p = [None, 1.0]
            result.append([len(x), x.mean(), len(y), y.mean(), self.cohen_d(x, y), y.mean()/x.mean(), p])
    ...
    def diff_by_signed_rank(self, data, pairs, target, features):
        result = []
        pairs = pairs.dropna(how='any')
        for f in features:
            y = data.loc[pairs.index, f]
            x = data.loc[list(pairs.iloc[:, 0]), f]
            try:
                stat, p = self.wilcoxon_rank_sum_test(x, y)
            except:
                stat, p = [None, 1.0]
    ...
    def variance_pca(self, data, max_pc=50, explained_variance_threshold=0.85, min_pc=3):
        ...
        for k in range(len(pca.explained_variance_ratio_)):
            ...
            if cumulative_explained_variance >= explained_variance_threshold:
                break
        return cumulative_explained_variance, pd.DataFrame(X[:, :max(k, min_pc)], index=data.index, columns=['PC'+str(i) for i in range(max(k, min_pc))])
